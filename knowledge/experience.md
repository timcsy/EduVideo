...
- **它如何發生：**把原本只發布 macOS 的 Electron 桌面版擴展到 Windows，前端與 IPC 不需要改，工作集中在主程序呼叫外部執行檔與打包的轉接層。
- **實際發生：**兩個錯誤在 macOS 上完全測不出來：whisper-cli 在 Windows 以系統字碼頁接收命令列參數，中文使用者路徑與繁中提示詞都會損壞；打包版替換 asar 路徑時只認 `/`，Windows 的反斜線路徑會讓 ffmpeg 找不到。
- **如何解決：**非 ASCII 資料改用 UTF-8 回應檔傳給子程序，路徑分隔符號兩種都處理；CI 在 Windows runner 上以中文路徑實際執行辨識，並檢查安裝檔的內容物。
- **教訓：**命令列、路徑與檔案系統慣例是平台轉接層的資料契約；只在開發平台通過的測試，證明的是開發平台的慣例，而不是目標平台的行為。
- **不適用於：**完全在瀏覽器內執行、不呼叫外部程序也不碰檔案系統路徑的前端邏輯。
- **來源：**commit b11fd71；[](episodes/2026-09-14-Windows桌面版支援.md)、[](history/004-從Tauri跨平台設定到Electron跨平台桌面主線.md)
...
| Windows、跨平台、子程序、路徑、打包 | [](episodes/2026-09-14-Windows桌面版支援.md) |
